...
def load_model_and_tokenizer(
    ...
    peft_adapter_path="./t5_chatbot_peft_adapter"
):
    """
    Load the trained model and tokenizer with PEFT adapter if available
    """
    print("[INFO] Loading model and tokenizer...")
    try:
        # Check if PEFT adapter exists
        is_peft_model = os.path.exists(peft_adapter_path)
        
        if is_peft_model:
            print("[INFO] Loading with PEFT adapter...")
        ...
        if is_peft_model:
            print("[INFO] Using PEFT adapter for parameter-efficient fine-tuning")
...
def evaluate_model(model, tokenizer, device, test_data_path="./HC_DATA/medquad_qa_pairs.csv", num_samples=None):
    ...
    num_samples_to_show = min(5, len(df))
    print(f"\n=== Sample Predictions (showing {num_samples_to_show}) ===")
    for i in range(num_samples_to_show):
        print(f"\nQ: {df.iloc[i]['question']}")
        print(f"True A: {df.iloc[i]['answer']}")
        print(f"Pred A: {predictions[i]}")
